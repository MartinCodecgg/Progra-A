...
begin
     Leer(mt,n,Vcli);
     InciA(mt,n);             //Recordar que cambiar el tipo o nombre a una variable es muy costoso, por ello intentar de no hacerlo,
     InciB(mt,Vcli,n);           //Y de hacerlo deberia recorrer el programa de pies a cabeza
     //Inciso C                   //Siempre que ponga una tipo byte a una variable pensar dos veces antes si va byte o word, porque luego el cambio
     Writeln('El promedio de compra por cliente es de: ',InciC(mt,n):8:2);          //es muy arriesgado y/o costoso
