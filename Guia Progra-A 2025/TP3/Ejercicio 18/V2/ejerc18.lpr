...
begin
     ...
     cantMaxOferta:=0;
     cantCliSoloOferta:=0;
                                //La mejor forma de inicializar un string o char es con el espacio vacio, semanticamente indica que solo se inicializo
                      //De todas formas no es correcto inicializar variables de esta forma solo para que el WHILE no valide con un valor basura
     repeat           //Es mejor usar el repeat en estos casos, leer ni bien empieza el repeat
           readln(arch,nombre);
           readln(arch,cant);
